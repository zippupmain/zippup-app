...
class PersonalSearchScreen extends StatefulWidget {
	const PersonalSearchScreen({super.key, required this.bookingId});
...
class _PersonalSearchScreenState extends State<PersonalSearchScreen> with TickerProviderStateMixin {
	late AnimationController _searchController;
	late Animation<double> _searchAnimation;
	StreamSubscription? _bookingSub;
	PersonalBooking? _booking;
	...
	void _listenForProviderAcceptance() {
		_bookingSub = FirebaseFirestore.instance
			...
			.listen((doc) {
				...
				final data = doc.data()!;
				final booking = PersonalBooking.fromJson(doc.id, data);
				
				setState(() => _booking = booking);
				
				// When provider accepts, navigate to live tracking
				if (booking.status == PersonalStatus.accepted && !_foundProvider) {
	...
	Widget build(BuildContext context) {
		return Scaffold(
			appBar: AppBar(
				title: const Text('👤 Finding Personal Service Provider'),
						...
						child: Column(
							...
							children: [
								...
								Text(
									_foundProvider 
										? '✅ Provider Found!'
										: '👤 Connecting to Personal Service Providers...',
									style: TextStyle(
										fontSize: 24,
										fontWeight: FontWeight.bold,
										color: _foundProvider ? Colors.green.shade700 : Colors.purple.shade700,
									),
									...
									child: Padding(
										...
										child: Column(
											...
											children: [
												Row(
													children: [
														Icon(Icons.person_pin, color: Colors.purple.shade700, size: 24),
								...
								Text(
									_foundProvider
										? 'Provider is preparing to assist you...'
										: 'We\'re connecting you with qualified personal service providers in your area.',
									style: TextStyle(
										fontSize: 16,
