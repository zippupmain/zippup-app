...
class RentalsProviderDashboardScreen extends StatefulWidget {
	const RentalsProviderDashboardScreen({super.key});
	@override
	State<RentalsProviderDashboardScreen> createState() => _RentalsProviderDashboardScreenState();
}

class _RentalsProviderDashboardScreenState extends State<RentalsProviderDashboardScreen> {
	...
	Future<void> _openAddProperty() async {
		await showModalBottomSheet(context: context, isScrollControlled: true, builder: (ctx) => _PropertyForm());
	}

	Future<void> _openAddVehicle() async {
		await showModalBottomSheet(context: context, isScrollControlled: true, builder: (ctx) => _VehicleForm());
	}
	...
	Widget build(BuildContext context) {
		...
		return DefaultTabController(
			length: 3,
			child: Scaffold(
				appBar: AppBar(title: const Text('Rentals Provider'), actions: [
				...
				body: TabBarView(children: [
					...
					Column(children: [
						Padding(
							...
							child: Row(children: [
								FilledButton.icon(onPressed: _openAddProperty, icon: const Icon(Icons.home_work), label: const Text('Add property')),
								const SizedBox(width: 8),
								FilledButton.icon(onPressed: _openAddVehicle, icon: const Icon(Icons.directions_car), label: const Text('Add vehicle')),
							]),
						),
						Expanded(
									...
									return ListView.separated(
										...
										itemBuilder: (context, i) {
											...
											return ListTile(
												...
												subtitle: Text(d['type'] == 'property' ? 'Property • ${(d['tenure'] ?? '').toString()}' : 'Vehicle • ${(d['timeframe'] ?? '').toString()}'),
					...
					FutureBuilder<QuerySnapshot<Map<String, dynamic>>>(
						...
						builder: (context, s) {
							...
							for (final d in s.data!.docs) {
								...
								if (st == 'completed' || st == 'delivered') { completed++; earnings += (m['price'] as num?)?.toDouble() ?? 0; }
							}
...
class _PropertyFormState extends State<_PropertyForm> {
	...
	String _tenure = 'monthly'; // daily | monthly | annual
	Uint8List? _imageBytes;
	bool _saving = false;

	Future<void> _pick() async {
		final file = await ImagePicker().pickImage(source: ImageSource.gallery, imageQuality: 85);
		if (file != null) { final bytes = await file.readAsBytes(); if (mounted) setState(() => _imageBytes = bytes); }
	}
	...
	Future<void> _save() async {
		...
		try {
			...
			String? url;
			if (_imageBytes != null) {
				final ref = FirebaseStorage.instance.ref('listings/$uid/property_${DateTime.now().millisecondsSinceEpoch}.jpg');
	...
	Widget build(BuildContext context) {
		return Padding(
			...
			child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
				TextField(controller: _title, decoration: const InputDecoration(labelText: 'Property title')),
				TextField(controller: _price, decoration: const InputDecoration(labelText: 'Price'), keyboardType: TextInputType.number),
				DropdownButtonFormField<String>(value: _tenure, items: const [
					DropdownMenuItem(value: 'daily', child: Text('Daily')),
					DropdownMenuItem(value: 'monthly', child: Text('Monthly')),
					DropdownMenuItem(value: 'annual', child: Text('Annual')),
				], onChanged: (v) => setState(() => _tenure = v ?? 'monthly'), decoration: const InputDecoration(labelText: 'Tenure')),
				const SizedBox(height: 8),
				Row(children: [IconButton(onPressed: _pick, icon: const Icon(Icons.image)), const Text('Add photos')]),
				const SizedBox(height: 12),
				FilledButton(onPressed: _saving ? null : _save, child: Text(_saving ? 'Saving...' : 'Save property')),
			]))
		);
	}
}
...
class _VehicleFormState extends State<_VehicleForm> {
	final _title = TextEditingController();
	final _price = TextEditingController();
	String _timeframe = 'daily'; // hourly | daily | weekly
	Uint8List? _imageBytes;
	bool _saving = false;
	...
	Future<void> _save() async {
		...
		try {
			...
			String? url;
			if (_imageBytes != null) {
				final ref = FirebaseStorage.instance.ref('listings/$uid/vehicle_${DateTime.now().millisecondsSinceEpoch}.jpg');
				await ref.putData(_imageBytes!, SettableMetadata(contentType: 'image/jpeg'));
				url = await ref.getDownloadURL();
			}
			await FirebaseFirestore.instance.collection('listings').add({
	...
	Widget build(BuildContext context) {
		return Padding(
			padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom, left: 16, right: 16, top: 16),
			child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
				TextField(controller: _title, decoration: const InputDecoration(labelText: 'Vehicle title')),
				TextField(controller: _price, decoration: const InputDecoration(labelText: 'Price'), keyboardType: TextInputType.number),
				DropdownButtonFormField<String>(value: _timeframe, items: const [
					DropdownMenuItem(value: 'hourly', child: Text('Hourly')),
					DropdownMenuItem(value: 'daily', child: Text('Daily')),
					DropdownMenuItem(value: 'weekly', child: Text('Weekly')),
				], onChanged: (v) => setState(() => _timeframe = v ?? 'daily'), decoration: const InputDecoration(labelText: 'Time frame')),
				const SizedBox(height: 8),
				Row(children: [IconButton(onPressed: _pick, icon: const Icon(Icons.image)), const Text('Add photos')]),
				const SizedBox(height: 12),
				FilledButton(onPressed: _saving ? null : _save, child: Text(_saving ? 'Saving...' : 'Save vehicle')),
			]))
		);
	}
}